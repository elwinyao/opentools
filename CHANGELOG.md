...
- `growth-tracker.js`：
  - `renderTrend()` 遍历全部记录（任一指标有值即计入），计算全局 `gT0`（最早记录日）/ `gT1`（最晚记录日）并传给三条曲线
  - `renderTrendSeries(all, s, gT0, gT1)` 改用全局时间范围：删除原「首条至末条」独立计算；X 轴刻度改为全局范围内每月 1 号 + 本图记录日期；X 坐标映射、图表宽度、滚动位置随之完全对齐
  - 某指标早期/后期无记录时自然留白，不影响数据展示
- 版本号：`growth-tracker.js?v=15` → `?v=16`；`sw.js CACHE_NAME` → `baby-tracker-v42`
