...
function loadSupabaseSDK() {
  ...
  _sbSdkPromise = new Promise(function(resolve, reject) {
    ...
    if (document.currentScript && document.currentScript.src) {
      var m = document.currentScript.src.match(/(.*\/)lib\/common-bundle\.js/i);
      if (m) base = m[1] + 'lib/';
    }
    s.src = base + 'supabase-js.min.js';
...
var LoginModalManager = (function() {
  ...
  function init(containerEl, callbacks) {
    ...
    _container.innerHTML =
      '<div class="modal-overlay" id="loginModal" style="display:none">' +
        '<div class="modal-box">' +
          ...
          '<button type="button" class="btn-block btn-login" id="loginBtn">🔑 登录</button>' +
    ...
    _container.querySelector('#loginBtn').addEventListener('click', _handleLogin);
...
async function restoreSession() {
  ...
  if (userInfo && reason === 'ok' && userInfo.access_token) { App.currentUser = userInfo; return { success: true, reason: 'ok' }; }
  if (userInfo && reason === 'ok' && userInfo.refresh_token) {
    App.currentUser = userInfo;
    var ok = await refreshAccessToken();
    if (ok) return { success: true, reason: 'ok' };
    App.currentUser = null;
    var sdkFallback = await _restoreFromSDK(userInfo);
    if (sdkFallback) return sdkFallback;
  ...
  if (reason === 'expired') {
    if (userInfo && userInfo.refresh_token) { App.currentUser = userInfo; var ok2 = await refreshAccessToken(); if (ok2) return { success: true, reason: 'ok' }; App.currentUser = null; }
  ...
  if (reason === 'no_data' || reason === 'no_token') { var sdkResult3 = await _restoreFromSDK(null); if (sdkResult3) return sdkResult3; }
...
async function refreshAccessToken(retries) {
  ...
  for (var attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (result.error || !result.data || !result.data.session) { if (attempt < retries) { await new Promise(function(r) { setTimeout(r, (attempt + 1) * 1000); }); continue; } return false; }
      ...
      await saveUserSecure(App.currentUser);
      return true;
    } catch(e) { Logger.warn('Token 刷新失败 (尝试 ' + (attempt + 1) + '/' + (retries + 1) + ')', e); if (attempt < retries) { await new Promise(function(r) { setTimeout(r, (attempt + 1) * 1000); }); continue; } return false; }
  }
  return false;
}
...
async function verifyAccessToken() {
  ...
  try { var result = await App.sbClient.auth.getUser(App.currentUser.access_token); return !result.error; } catch(e) { Logger.warn('Token 验证失败', e); return false; }
}

function skipLogin() { App.currentUser = null; clearUserSecure(); sessionStorage.setItem('bt_skip_login', '1'); updateSyncStatus('offline'); clearUserDisplay(); }
...
function loadXlsxModule(callback) {
  ...
  // 推导 lib 目录路径，兼容 index.html (lib/) 和 baby-tracker.html (../lib/)
  var libBase = '../lib/';
  if (document.currentScript && document.currentScript.src) {
    var m = document.currentScript.src.match(/(.*\/)lib\/common-bundle\.js/i);
    if (m) libBase = m[1] + 'lib/';
  }
  var failed = false;
...
function registerSW() {
  ...
  navigator.serviceWorker.register('/sw.js', { scope: '/' }).then(function(reg) { console.log('[PWA] SW 注册成功:', reg.scope); }).catch(function(err) { Logger.warn('PWA Service Worker 注册失败', err); });
}
