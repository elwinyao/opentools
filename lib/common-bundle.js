...
function loadSupabaseSDK() {
  ...
  _sbSdkPromise = new Promise(function(resolve, reject) {
    ...
    // 根据页面路径深度选择 lib 目录：根目录用 lib/，子目录用 ../lib/
    var depth = (location.pathname.match(/\//g) || []).length - 1;
    var base = depth > 0 ? '../lib/' : 'lib/';
...
function loadXlsxModule(callback) {
  ...
  // 推导 lib 目录路径，根据页面路径深度自动选择
  var depth = (location.pathname.match(/\//g) || []).length - 1;
  var libBase = depth > 0 ? '../lib/' : 'lib/';
  if (document.currentScript && document.currentScript.src) {
    var m = document.currentScript.src.match(/(.*\/)lib\/common-bundle\.js/i);
...
function registerSW() {
  ...
  // 根据页面深度找到根目录的 sw.js（子目录页面需加 ../）
  var depth = (location.pathname.match(/\//g) || []).length - 1;
  var swPath = (depth > 0 ? '../' : './') + 'sw.js';
  navigator.serviceWorker.register(swPath, { scope: depth > 0 ? '../' : './' }).then(function(reg) { console.log('[PWA] SW 注册成功:', reg.scope); }).catch(function(err) { Logger.warn('PWA Service Worker 注册失败', err); });
}
