...
function friendlyNetworkError(e) {
  ...
  // RLS 42501：请求未携带有效 JWT（如会话被互踢/本地恢复的旧 token 已失效），写入被行级安全拒绝
  if (/row-level security|42501|permission denied for table/i.test(msg)) return '登录已失效，请重新登录后再试';
...
function initSupabase() {
  ...
  App.sbClient.auth.onAuthStateChange(function(event, session) {
    if (event === 'TOKEN_REFRESHED' && session && App.currentUser) {
      ...
      // 会话已被服务端作废（如多端互踢/refresh_token 400）：清空同步队列，
      // 防止残留队列在无有效 JWT 下反复触发 upsert → RLS 42501 刷屏
...
async function refreshAccessToken(retries) {
  ...
  for (var attempt = 0; attempt <= retries; attempt++) {
    try {
      // 优先走 SDK 内部会话（无参 refreshSession，token 永远最新）；仅当 SDK 无内部会话时用本地 token 一次性兜底
      var result = await App.sbClient.auth.refreshSession();
      if (result.error) {
        var errMsg = result.error.message || '';
        var status = result.error.status;
        var isNoSession = /no (current )?session/i.test(errMsg);
        // 网络类错误（无状态码/5xx/超时/abort/fetch）：服务端可能已成功轮换 token（响应在链路上丢失），
        // 此时再用本地旧 refresh_token 兜底必然 400，会把仍有效的会话误判成失效。
        // 手机弱网/20s 超时高发，表现为"过会儿要求重新登录"。因此：不判死、不兜底，标记网络错误重试；耗尽后返回 'network' 由上层保留会话。
        var isNetworkErr = !isNoSession && (!status || status >= 500 || /timeout|abort|fetch|network|signal/i.test(errMsg));
        if (isNetworkErr) {
        ...
        // 凭证确实无效（400/401），或 SDK 内部无会话：才用本地 refresh_token 兜底一次
        if (((status === 400 || status === 401) || isNoSession) && App.currentUser.refresh_token) {
          result = await App.sbClient.auth.refreshSession({ refresh_token: App.currentUser.refresh_token });
          if (result.error) { Logger.warn('refresh_token 已失效（' + result.error.status + '），需重新登录'); return false; }
        } else {
          Logger.warn('Token 刷新失败: ' + errMsg); return false;
        }
      }
...
// 确保 SDK 持有有效会话后返回 true：
// 1) SDK 内部已有未过期 session → 直接通过；
// 2) 仅本地恢复的 currentUser 有 token、SDK 内部无 session（_tryQuickPath/_handleSessionOk 只写了 App.currentUser，
//    未注入 SDK）→ 用本地 access_token/refresh_token 调 setSession 注入，注入成功视为有效；
// 3) 注入也失败（token 已被服务端作废，如多端互踢/refresh_token 400）→ 返回 false，调用方跳过写入，
//    避免 PostgREST 以 anon 身份执行 → RLS `auth.uid() = user_id` 恒 false → 42501 刷屏
async function _ensureValidSdkSession() {
  if (!App.sbClient || !App.currentUser) return false;
  var sess = App.sbClient.auth.getSession();
  if (sess.data && sess.data.session) {
    var expiresAt = sess.data.session.expires_at;
    if (!expiresAt || Date.now() / 1000 < expiresAt - 60) return true; // 60s 余量：过期前视为有效
  }
  if (App.currentUser.access_token) {
    try {
      var r = await App.sbClient.auth.setSession({ access_token: App.currentUser.access_token, refresh_token: App.currentUser.refresh_token || '' });
      if (r.data && r.data.session) return true;
    } catch(e) { Logger.warn('SDK 会话注入失败', e); }
  }
  return false;
}
...
async function syncRecordToCloud(record, dateStr, opts) {
  ...
  if (!App.sbClient || !App.currentUser) return;
  try {
    // 会话有效性前置校验：本地凭据已失效（互踢/refresh 400）时不再硬发请求，
    // 否则 PostgREST 以 anon 身份执行 → RLS `auth.uid() = user_id` 恒 false → 42501 刷屏
    if (!await _ensureValidSdkSession()) {
      Logger.warn('会话已失效（RLS 防护），跳过云端写入', record && record.id);
      return; // 不入队、不重试：避免无效会话下无限 42501
    }
...
async function deleteRecordFromCloud(recordId, opts) {
  opts = opts || {};
  if (!App.sbClient || !App.currentUser) return;
  if (!await _ensureValidSdkSession()) { Logger.warn('会话已失效（RLS 防护），跳过云端删除', recordId); return; }
