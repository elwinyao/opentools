...
App.CONFIG = {
  ...
  // Realtime 连接状态轮询间隔（毫秒）：定时检查 isConnected() 同步 UI 状态（移动端 WS 长连接易被挂起/重置，缩短轮询可更快恢复）
  REALTIME_STATUS_POLL_MS: 30000,                      // 30 秒

  // Realtime 自动重连冷却时间（毫秒）：防抖，避免频繁 removeChannel + subscribe
  REALTIME_RECONNECT_COOLDOWN_MS: 10000,               // 10 秒
...
// 全局 fetch 包装：所有 Supabase 请求统一 20s 超时，超时抛 AbortError。
// 海外链路（supabase.co 非国内区域）弱网时 fetch 可能挂起几十秒甚至不返回，
// 加超时后用户能立即得到明确提示，而不是一直转圈 / 最后才报 "Failed to fetch"
function fetchWithTimeout(input, init) {
  var controller = new AbortController();
  var timer = setTimeout(function() {
    // 带 reason 的 abort：让 fetch 拒绝时 name 为 TimeoutError，message 明确为「请求超时」，
    // 避免 Chrome 126+ 无 reason abort 时出现 "signal is aborted without reason" 之类的英文裸奔消息
    try { controller.abort(new DOMException('请求超时', 'TimeoutError')); }
    catch (err) { controller.abort(); } // 老环境不支持带参 abort 时降级
  }, App.CONFIG.FETCH_TIMEOUT_MS || 20000);
  var opts = Object.assign({}, init || {}, { signal: controller.signal });
  return fetch(input, opts).finally(function() { clearTimeout(timer); });
}

// 把 fetch/网络错误转成用户可理解的文案
function friendlyNetworkError(e) {
  if (!e) return '未知错误';
  var msg = e.message || '';
  // name 可能因 SDK 包装丢失（如仅保留 message），故同时按消息内容兜底匹配
  if (e.name === 'AbortError' || e.name === 'TimeoutError' || /abort|timeout|time\s*out/i.test(msg)) return '网络连接超时，请检查网络后重试';
  // 注意：不依赖 instanceof TypeError——supabase SDK 会把网络错误包装成 AuthError（extends Error），
  // 仅凭消息内容匹配，否则 "Failed to fetch" 会漏掉而裸显英文原文
  if (/failed to fetch|networkerror|network error|load failed|无法连接/i.test(msg)) return '网络连接失败，请检查网络后重试';
  return msg || '未知错误';
}
...
function initSupabase() {
  ...
  // SDK 默认把完整会话明文写入 localStorage（key: sb-<ref>-auth-token），与项目加密存储重复且扩大泄露面。
  // 注入加密转发 storage：SDK 状态机照常工作（getSession/查询/刷新均正常），
  // 但落盘内容经 AES-GCM 加密；HTTP 局域网等无 crypto.subtle 环境自动退回 SDK 原生行为
  var encryptedStorage = (function() {
    function rawGet(key) { try { return localStorage.getItem(key); } catch(e) { return null; } }
    function rawSet(key, value) { try { localStorage.setItem(key, value); } catch(e) {} }
    ...
    return {
      getItem: async function(key) {
        try {
          var v = rawGet(key);
          if (!v) return null;
          if (v.charAt(0) === '{') return v;                       // 兼容历史明文数据
          if (!(window._secureCrypto && window._secureCrypto.decrypt)) return null;
          return await window._secureCrypto.decrypt(v) || null;
        } catch(e) { return null; }
      },
      setItem: async function(key, value) {
        try {
          if (window._secureCrypto && window._secureCrypto.encrypt) { rawSet(key, await window._secureCrypto.encrypt(value)); return; }
        } catch(e) {}
        rawSet(key, value);                                        // 加密不可用时兜底明文，保证功能不中断
      },
      removeItem: function(key) { rawRemove(key); }
    };
  })();
  App.sbClient = supabase.createClient(App.SUPABASE_URL, App.SUPABASE_PUBLISHABLE_KEY, {
    auth: { autoRefreshToken: true, persistSession: true, storage: encryptedStorage },
    realtime: { params: { eventsPerSecond: App.CONFIG.REALTIME_EVENTS_PER_SECOND } },
    // 注意：不要添加自定义请求头（如 X-Requested-With）。
    // Safari 对 CORS 预检（OPTIONS）校验极严，自定义头会强制先发预检，
    // 即使服务端返回 200 也可能判定 access control checks 失败 → "Failed to fetch"
    // （supabase/supabase#20982，Safari 登录/登出/查询全挂的已知案例）
...
(function() {
  ...
  // 登出闸门（时间窗式）：clearUserSecure 删除种子时记录时间戳，
  // 短窗口内拦截在途异步操作（如 TOKEN_REFRESHED 回调）重新生成/写回种子形成孤儿数据；
  // 窗口过后自动失效——登出后不刷新页面直接重新登录也能正常重建种子
  var _seedPurgedAt = 0;
  var SEED_PURGE_WINDOW_MS = 5000;

  async function _getKeyMaterial() {
    if (_seedPurgedAt && (Date.now() - _seedPurgedAt) < SEED_PURGE_WINDOW_MS) throw new Error('seed purged on logout');
  ...
  // 供 Supabase SDK storage 适配器复用 AES-GCM 加解密（见 initSupabase 的 encryptedStorage）
  window._secureCrypto = { encrypt: encrypt, decrypt: decrypt };
  ...
  window.clearUserSecure = function() {
    ...
    // 加密种子随登出一并清除（localStorage 备份 + IndexedDB 主存储）：
    // 密文已删，seed 单独留存无功能价值；删除后下次登录自动生成新种子，无副作用
    _seedPurgedAt = Date.now();
    _cachedKey = null;
    localStorage.removeItem(SEED_FALLBACK_KEY);
    ...
    try {
      _openDB().then(function(db) {
        return new Promise(function(resolve) {
          var tx = db.transaction(SEED_STORE, 'readwrite');
          tx.objectStore(SEED_STORE).clear();
...
var LoginModalManager = (function() {
  ...
  async function _handleLogin() {
    ...
    try {
      ...
      if (result.error) { errorEl.textContent = '登录失败：' + friendlyNetworkError(result.error); errorEl.style.color = '#e74c3c'; return; }
    ...
    } catch(e) { Logger.error('登录失败', e); errorEl.textContent = '登录失败：' + friendlyNetworkError(e); errorEl.style.color = '#e74c3c'; }
    finally {
  ...
  async function _handleSignup() {
    ...
    try {
      ...
      if (result.error) { errorEl.textContent = '注册失败：' + friendlyNetworkError(result.error); errorEl.style.color = '#e74c3c'; return; }
      var user = result.data.user;
      var session = result.data.session;
      if (user && session) {
        App.currentUser = { id: user.id, email: user.email, access_token: session.access_token, refresh_token: session.refresh_token, loginAt: Date.now() };
    ...
    } catch(e) { Logger.error('注册失败', e); errorEl.textContent = '注册失败：' + friendlyNetworkError(e); errorEl.style.color = '#e74c3c'; }
    finally {
...
async function _handleSessionOk(user) {
  ...
  if (user.refresh_token) {
    ...
    var r = await refreshAccessToken();
    if (r === true) { sessionStorage.setItem('bt_session_verified', String(Date.now())); return { success: true, reason: 'ok' }; }
    // 网络/超时类错误：会话凭证仍有效，保留登录态与本地数据，不弹"重新登录"（等网络恢复后自动恢复云端）
    if (r === 'network') return { success: true, reason: 'network' };
    App.currentUser = null;
  }
...
async function _handleSessionExpired(user) {
  if (user && user.refresh_token) {
    ...
    var r = await refreshAccessToken();
    if (r === true) { sessionStorage.setItem('bt_session_verified', String(Date.now())); return { success: true, reason: 'ok' }; }
    // 网络/超时类错误：凭证未失效（只是网络不通），保留登录态与本地数据，不弹"重新登录"
    if (r === 'network') return { success: true, reason: 'network' };
    App.currentUser = null;
  }
...
// 单飞互斥：同一时刻最多只有一个刷新请求在途（模块级共享 Promise）。
// 4 连发 400 的根因就是无互斥下 2 个调用方并发 × 每次 2 连发（无参+兜底）→ 同一 refresh_token 被并发使用。
// Supabase refresh token 单次使用+轮换制，并发必 400（latency 0 秒拒）。
var _tokenRefreshingPromise = null;

// 返回三态：true=刷新成功；false=凭证确实失效（清会话引导重新登录）；'network'=网络/超时类错误（会话仍有效，保留，勿判死）
async function refreshAccessToken(retries) {
  if (retries === undefined) retries = 1;
  if (!App.sbClient || !App.currentUser) return false;
  if (!App.currentUser.refresh_token) return false;
  // 单飞：已有刷新在途则直接复用其结果，不再发新请求
  if (_tokenRefreshingPromise) return _tokenRefreshingPromise;
  _tokenRefreshingPromise = _doRefreshAccessToken(retries).finally(function() {
    _tokenRefreshingPromise = null;
  });
  return _tokenRefreshingPromise;
}

async function _doRefreshAccessToken(retries) {
  for (var attempt = 0; attempt <= retries; attempt++) {
    try {
      // 直接传本地 refresh_token（单次、可控，不依赖 SDK 内部会话状态）
      ...
      if (result.error) {
        ...
        // 网络类错误（无状态码/5xx/超时/abort/fetch）：服务端可能已成功轮换 token（响应在链路上丢失），
        // 不判死、不用旧 token 重复兜底（避免 400 竞态误杀会话）。重试耗尽后返回 'network' 由上层保留会话。
        var isNetworkErr = !status || status >= 500 || /timeout|abort|fetch|network|signal/i.test(errMsg);
        if (isNetworkErr) {
          if (attempt < retries) { await new Promise(function(r) { setTimeout(r, (attempt + 1) * 1000); }); continue; }
          Logger.warn('Token 刷新网络错误（保留会话）: ' + errMsg); return 'network';
        }
        // 400/401：凭证确实失效（互踢/已被轮换），单次失败即判死，不再二连发
        Logger.warn('refresh_token 已失效（' + status + '），需重新登录'); return false;
      }
      ...
      var isNet = !e || !e.status || /timeout|abort|fetch|network|signal/i.test(eMsg);
      Logger.warn('Token 刷新异常 (尝试 ' + (attempt + 1) + '/' + (retries + 1) + ')', e);
      if (isNet) { if (attempt < retries) { await new Promise(function(r) { setTimeout(r, (attempt + 1) * 1000); }); continue; } return 'network'; }
      if (attempt < retries) { await new Promise(function(r) { setTimeout(r, (attempt + 1) * 1000); }); continue; }
      return false;
    }
  }
...
async function logout() {
  ...
  clearApiCache();
  if (App.sbClient) {
    // 登出前先尝试刷新一次 token：手机后台冻结时 access token 可能已过期，
    // 直接 signOut 会被 GoTrue 拒绝（403，服务端 session 吊销失败）。
    // 网络正常时 <1s 完成；弱网失败则忽略，不阻塞登出。
    try { await refreshAccessToken(0); } catch (e) {}
    // signOut 失败（弱网/超时/403）不阻塞本地登出，保证本地清理始终执行
    try { await App.sbClient.auth.signOut(); } catch (e) { Logger.warn('signOut 请求失败（忽略，继续本地清理）', e); }
  }
  App.currentUser = null;
  clearUserSecure();
  sessionStorage.removeItem('bt_session_verified');
  // 清所有页面数据键 + 同步队列，防止换账号/未登录时旧数据串显。
  // 注意：三个页面的 localStorage 键必须在这里集中清——若依赖各页 onLogout 钩子，
  // 从 index 登出时其他页面 JS 未加载、钩子未注册，专属键会残留导致串显
  ...
  localStorage.removeItem('baby_vaccine_data');
  // 页面钩子：各页清本页内存数据并重渲染空视图（已加载页面的视图立即变空）
...
async function loadMonthFromCloud(year, month) {
  ...
  try {
    ...
    // 遍历「本地当月日期 ∪ 云端当月日期」：云端缺失的本地日期用空数组合并，
    // 已同步（有 updatedAt）且云端已删的记录被丢弃，避免月度汇总残留
    var localDates = Object.keys(App.allData).filter(function(d) { return d >= firstDay && d <= lastDay; });
    var allDates = localDates.slice();
    Object.keys(cloudByDate).forEach(function(d) { if (allDates.indexOf(d) < 0) allDates.push(d); });
...
async function syncRecordToCloud(record, dateStr, opts) {
  ...
  if (!App.sbClient || !App.currentUser) return;
  try {
    // 显式传 user_id（与 JWT 的 auth.uid() 一致），即使数据库 DEFAULT 未生效也不会 NOT NULL 违反；仍兼容 RLS
