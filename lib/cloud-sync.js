...
async function loadDayFromCloud(dateStr) {
  ...
  // 智能合并：逐条比对 updatedAt，云端更新才覆盖本地，本地更新的保留
  ...
  cloudRecords.forEach(function(cr) {
    ...
    if (!lr) {
      // 本地没有的新记录，直接加入
      localRecords.push(cr);
    } else {
      // 两边都有：比较 updatedAt，云端更新才覆盖
      ...
      // 否则保留本地版本（本地编辑过但尚未同步到云端的情况）
    }
    localMap[cr.id] = true; // 标记已在云端
  });

  allData[dateStr] = localRecords.sort(function(a, b) {
