...
function scheduleHourlyRefresh(key) {
  ...
  if (!last) {
    ...
    setTimeout(function() { localStorage.setItem(key, Date.now()); location.reload(); }, 3600000);
    return;
  }
  ...
  setTimeout(function() { localStorage.setItem(key, Date.now()); location.reload(); }, 3600000 - elapsed);
}
