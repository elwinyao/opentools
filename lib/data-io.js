// ==================== 数据导入/导出模块 ====================
// 依赖：app-namespace.js, storage.js (saveData, getDayData), utils.js
// 外部依赖：updateSyncStatus(), syncRecordToCloud(), renderRecords(), renderSummary()

// 导出数据为 JSON 文件
function exportData() {
  exportJSON(App.allData, '宝宝作息数据_' + currentDateBJ() + '.json');
}
...
async function importData(event) {
  ...
  reader.onload = async function(e) {
    try {
      ...
      Logger.fatal('文件格式不正确，请选择导出的 .json 文件');
    }
    event.target.value = '';
  };
